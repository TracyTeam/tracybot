...
function buildTaskletMessages(tasklet_str: string): { messages: TaskletMessage[], title: string } {
  ...
  if (tasklet_obj?.planOutputs && Array.isArray(tasklet_obj.planOutputs)) {
    tasklet_obj.planOutputs.forEach((plan: any) => {
      if (plan.prompt) {
        messages.push({ stage: "plan", type: "prompt", message: plan.prompt, questions: undefined });
      }

      if (plan.response) {
        messages.push({ stage: "plan", type: "response", message: plan.response, questions: plan.questions ?? undefined });
      }
    });
  }
  ...
  if (!tasklet_obj.buildOutput) {
    ...
    messages.push({ stage: "build", type: "prompt", message: tasklet_obj.buildOutput?.prompt, questions: tasklet_obj.buildOutput?.questions ?? undefined });
    messages.push({ stage: "build", type: "response", message: tasklet_obj.buildOutput?.response, questions: tasklet_obj.buildOutput?.questions ?? undefined });
  }
